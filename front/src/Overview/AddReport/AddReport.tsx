...
const AddReport: React.FC<{ onReportAdded: () => void }> = ({
    ...
    return (
        <div className="flex flex-col bg-secondary-400 px-6 py-4 overflow-hidden rounded w-full mb-4 sm:mb-0 sm:mr-4">
            ...
            <button
                disabled={isLoading}
                className="block duration-300 ease-in-out text-gray-200 text-xl px-4 py-2 rounded-lg bg-primary-400 disabled:opacity-25 w-full opacity-100 transition-all transform hover:-translate-y-1 focus:outline-none mb-2"
                type="button"
                onClick={() => addReport()}
            >
                {isLoading ? (
